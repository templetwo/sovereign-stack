...
"""
Pending write queue for the OpenAI bridge.

Ring 2 tool calls from OpenAI never touch the Stack directly.
They create a proposal here. Anthony reviews and approves.
Approval and commit are separate steps.

Proposal is not memory.
Approval is not commitment.
OpenAI write intent is not Stack truth.
"""
...
class Proposal:
    ...
    commit_result: dict | None = None  # populated after commit (mocked in Phase 2)
...
def validate_pending_write(proposal: Proposal) -> list[str]:
    """
    Validate a proposal against policy.

    Returns a list of validation errors. Empty list = valid.
    Does NOT raise — caller decides whether to reject or dry-run report.
    """
    errors: list[str] = []
    ...
    if proposal.tool not in COMMIT_TARGETS:
...
def create_pending_write(
    ...
    """
    Create a Ring 2 write proposal.

    If dry_run=True: validates and returns the Proposal object without writing to disk.
    If validation fails: raises ValidationError (regardless of dry_run).

    The underlying Stack tool is never called. Only a proposal file is created.
    """
    ...
    proposal = Proposal(
        ...
        tool=tool_name,
        arguments=args,
        commit_target=COMMIT_TARGETS.get(tool_name, tool_name),
    ...
    errors = validate_pending_write(proposal)
    if errors:
        append_audit_event(
    ...
    if not dry_run:
        ...
        logger.info(
            "Proposal created: %s | tool=%s risk=%s layer=%s",
            proposal_id,
            tool_name,
            risk_level.value,
            proposed_layer,
...
def _load_proposal(proposal_id: str) -> tuple[Proposal, Path]:
    """Find and load a proposal file by its proposal_id prefix or full id."""
    PENDING_DIR.mkdir(parents=True, exist_ok=True)
    matches = list(PENDING_DIR.glob(f"*{proposal_id[:8]}*.json"))
    if not matches:
        raise FileNotFoundError(f"No proposal found for id: {proposal_id}")
    if len(matches) > 1:
        # Prefer exact match on full id
...
# Mirrors bridge_core's set for the same reason VERIFIED_BY_TARGETS is scoped:
# since `_reject_unknown_params`, forwarding an undeclared parameter turns
# every commit carrying it into a hard Stack-side error.
...
def _precondition_check(proposal: Proposal) -> list[str]:
    """
    Run all pre-commit guards. Returns a list of blocking reasons.
    Empty list = safe to commit.
    """
    errors: list[str] = []
    ...
    if proposal.commit_target not in _ALLOWED_COMMIT_TARGETS:
    ...
    if proposal.risk_level == RiskLevel.CRITICAL and proposal.compass_check_result != "PROCEED":
        errors.append(
            ...
            "call compass_check, include the result in the proposal, then re-approve"
        )
...
def commit_pending_write(proposal_id: str, live: bool = False) -> Proposal:
    ...
    live=False (default): dry-run — returns proposal with would_call info, no Stack write.
    ...
    # Verify chain is still intact after the commit event
    ok, msg = verify_chain()
    if not ok:
...
# THE REJECT-EDGE STATE MACHINE. Imported from bridge_core so the two
# substrates cannot drift: this module and bridge_core/pending_writes.py are
# separate implementations of the same governance, and the last time a
# guarantee held on one and not the other (provenance passthrough, 2026-08-03
# -> 2026-08-30) it was "true for grok and FALSE for openai" for 27 days
# without anybody noticing. A guarantee that holds on one substrate is not a
# guarantee; it is a guarantee-shaped belief.
from bridge_core.pending_writes import (  # noqa: E402
    _REJECTABLE_STATUSES,
    _REVOCABLE_STATUSES,
    _reject_refusal_message,
)
...
def reject_pending_write(
    ...
    *,
    ...
    2026-05-25/27) have therefore sat in the queue with no exit, and ``reject``
    answered "Cannot reject proposal in status 'approved'".
    ...
    chronicle, and a queue that says "rejected" over a chronicle that says
    "written" is a worse record than no edge at all.

    ``commit_result`` IS LEFT UNTOUCHED — it is the evidence of what actually
    happened at the Stack, and a revocation must not edit that history.
    """
    ...
    if prior_status == "approved":
        ...
        logger.info(
            "Proposal APPROVAL REVOKED: %s by %s (was approved by %s at %s) reason=%s",
            proposal_id,
            rejected_by,
            prior_reviewed_by,
            prior_reviewed_at,
...
def needs_revision_pending_write(proposal_id: str, notes: str, *, actor: str) -> Proposal:
    ...
    logger.info("Proposal needs revision: %s", proposal_id)
    return proposal


def list_pending_writes(status: str | None = None) -> list[dict]:
    """
    List proposals, optionally filtered by status.
    Returns a summary list (not full proposal objects).
    """
    PENDING_DIR.mkdir(parents=True, exist_ok=True)
    results = []
