    ...
    return (
        [
            ...
            Tool(
                ...
                description=(
                    "Archive verbatim bytes that would otherwise die when the context "
                    "window closes: your own in-conversation drafts and iterations, "
                    "intermediate work, or an external model's full delivered output. "
                    "Content-addressed and hash-verified, stored separate from the curated "
...
def _start_here_text() -> str:
    ...
    return (
        ...
        "  • A summary is not the artifact. When bytes matter (another model's\n"
        "    full output, a delivered file, or your own in-conversation drafts\n"
        "    and iterations that should outlive this window), archive_exchange\n"
        "    the bytes first, then record_insight a summary that references the\n"
        "    returned archive_id. Group an iterative trajectory with a shared\n"
        "    conversation_id so the whole build is recoverable, not just the\n"
        "    endpoint. recall_exchange re-reads and re-hashes on the way back,\n"
        "    so a chronicle claim can never silently stand in for a missing\n"
...
# the first time. That serialization is held where it belongs — in the SYNC
# layer, inside the chronicle mutators themselves (provenance.chronicle_write_
# lock, taken by record_insight, the thread mutators, metabolism's whole-file
# rewrites, and the ledger appends). It is deliberately NOT held here: an
# asyncio.Lock releases on CancelledError (a client disconnect would drop it
# while the worker thread is still mid read-modify-write), and every inline
# caller — close_session, resolve_thread, a daemon — bypasses the dispatcher
# entirely. A second lock in the wrong layer is a liability, not defense in
# depth.
...
_ADDITIONAL_ALLOWED_PARAMS: dict[str, frozenset[str]] = {
    "record_insight": frozenset({"session_token_id", "source_instance"}),
}
...
"""
    ...
    # supersede_insight takes the chronicle write lock in the sync layer.
    ...
    # chronicle — the receipt re-hash, multiplied. link_threads takes the
