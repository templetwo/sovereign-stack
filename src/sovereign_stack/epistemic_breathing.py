...
def classify_query(query: str) -> Tuple[str, float, str]:
    ...
    if re.search(r'\b(i|my|me|myself)\b', query_lower) and \
       re.search(r'\b(feel|felt|afraid|scared|angry|sad|hurt|confused|lost|alone|broken)\b', query_lower):
        witness_score += 2
...
def breathe_query(query: str, results: List[Dict]) -> List[Dict]:
    ...
    if signal == "OPEN":
        ...
        other = [r for r in results if r.get("layer") not in ("ground_truth", "hypothesis", "open_thread")]
