...
**Status:** COMPLETE and shipped (generic mechanism, INERT — no record is designated). v1.9.0 shipped the coupling engine (§5.3 invariant at the chokepoint for `recall_insights`/`load_entries` + scribe; §5.6 audit). v1.10.0 added the two-word subject/emotion index (§5.1-extended), the consent gate (§5.4 — MCP tools `list_protected_thresholds`/`open_protected_record`/`decline_protected_record`; `audit_threshold` flags a content-leaking threshold), the boot line on both doors (§5.5), and CLOSED the §5.4 bypass perimeter: `inspect_claim` couples its surfaced entry, `walk_lineage` withholds previews, `dashboard.read_chronicle_tail` withholds, `season_review` filters protected out (`resolve_claim` gained `couple=False`; surfacing callers opt in). No read path now returns protected content bare.
**Phase 3 (designating the first real record) is HARD-GATED on ONE remaining precondition: Anthony's explicit, separate yes.** The bypass-path precondition is met. Until he says go, no record is protected and nothing can leak. `designate_protected` stays human-gated and library-only by design.
