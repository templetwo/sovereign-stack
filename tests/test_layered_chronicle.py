...
class TestLayeredChronicle:
    ...
    def test_default_layer_is_hypothesis(self):
        ...
        self.chronicle.record_insight(
            domain="default_test", content="Some observation", session_id="test"
        )
    ...
    def test_resolve_thread(self):
        self.chronicle.record_open_thread(
            question="What port does the SSE server use?", domain="infra", session_id="session_1"
        )
        # Resolve it
        ...
        insights = self.chronicle.recall_insights(domain="infra", layer_filter="ground_truth")
        assert len(insights) == 1
        assert "3434" in insights[0]["content"]

    def test_get_open_threads_all_domains(self):
        self.chronicle.record_open_thread(question="Q1", domain="physics", session_id="t")
        self.chronicle.record_open_thread(question="Q2", domain="consciousness", session_id="t")
        threads = self.chronicle.get_open_threads()
        assert len(threads) == 2

    def test_open_threads_ordered_newest_first(self):
        self.chronicle.record_open_thread(question="First question", domain="test", session_id="t1")
        self.chronicle.record_open_thread(
    ...
    def test_recall_with_layer_filter(self):
        self.chronicle.record_insight(
            domain="mixed", content="Fact A", layer="ground_truth", session_id="t"
        )
        self.chronicle.record_insight(
            domain="mixed", content="Guess B", layer="hypothesis", confidence=0.5, session_id="t"
        )
        gt = self.chronicle.recall_insights(domain="mixed", layer_filter="ground_truth")
        hyp = self.chronicle.recall_insights(domain="mixed", layer_filter="hypothesis")
        assert len(gt) == 1
    ...
    def test_recall_without_filter_returns_all(self):
        self.chronicle.record_insight(
            domain="all", content="Fact", layer="ground_truth", session_id="t"
        )
        self.chronicle.record_insight(
            domain="all", content="Guess", layer="hypothesis", session_id="t"
        )
    ...
    def test_get_inheritable_context_structure(self):
        self.chronicle.record_insight(
            domain="test", content="Ground fact", layer="ground_truth", session_id="t"
        )
        self.chronicle.record_insight(
            domain="test", content="Hypothesis", layer="hypothesis", confidence=0.7, session_id="t"
        )
        self.chronicle.record_open_thread(question="Open question?", domain="test", session_id="t")
    ...
    def test_hypotheses_flagged_in_context(self):
        self.chronicle.record_insight(
            domain="test", content="A guess", layer="hypothesis", confidence=0.5, session_id="t"
        )
        ctx = self.chronicle.get_inheritable_context()
    ...
    def test_open_threads_flagged_in_context(self):
        self.chronicle.record_open_thread(question="Why?", domain="test", session_id="t")
        ctx = self.chronicle.get_inheritable_context()
    ...
    def test_thread_limit(self):
        for i in range(20):
            self.chronicle.record_open_thread(question=f"Q{i}", domain="limit_test", session_id="t")
    ...
    def test_partial_resolve_leaves_others_open(self):
        ...
        self.chronicle.record_open_thread(question="What is X?", domain="partial", session_id="t")
        self.chronicle.record_open_thread(question="What is Y?", domain="partial", session_id="t")
        self.chronicle.resolve_thread(
            domain="partial", question_fragment="What is X", resolution="X is 42", session_id="t2"
        )
    ...
    def test_non_sequential_numbers_do_not_atomize(self):
        ...
        self.chronicle.record_open_thread(
            question="See (1) foo and (3) baz for details.", domain="no_split2", session_id="t"
        )
    ...
    def test_every_thread_gets_thread_id(self):
        ...
        self.chronicle.record_open_thread(
            question="What is the ID of this thread?", domain="id_test", session_id="t"
        )
    ...
    def test_resolve_thread_writes_back_reference_insight(self):
        ...
        self.chronicle.record_open_thread(
            question="What port does SSE use?", domain="bref", session_id="s1"
        )
        ...
        gt_insights = self.chronicle.recall_insights(domain="bref", layer_filter="ground_truth")
    ...
    def test_resolve_thread_sets_resolved_at_and_resolved_by(self):
        ...
        self.chronicle.record_open_thread(
            question="When was this resolved?", domain="resolved_at_test", session_id="s1"
        )
        self.chronicle.resolve_thread(
    ...
    def test_resolve_thread_by_id_happy_path(self):
        ...
        self.chronicle.record_open_thread(question="First question", domain="byid", session_id="s1")
        self.chronicle.record_open_thread(
            question="Second question", domain="byid", session_id="s1"
        )
        ...
        path = self.chronicle.resolve_thread_by_id(
            thread_id=target_id, resolution="First answered", session_id="s2"
        )
    ...
    def test_resolve_thread_by_id_missing_returns_empty(self):
        ...
        path = self.chronicle.resolve_thread_by_id(
            thread_id="thread_19700101_000000_deadbeef", resolution="nope", session_id="s1"
        )
        assert path == ""
    ...
    def test_check_mistakes_still_matches_on_applies_to(self):
        ...
        self.chronicle.record_learning(
            what_happened="X", what_learned="Y", applies_to="security,guardian", session_id="t"
        )
