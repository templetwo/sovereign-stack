...
run_check(
    "Categories correct",
    lambda: set(list_categories())
    == {"emotional", "emergence", "recursion", "memory", "threshold"},
)
...
run_check(
    "Transition to FIRST_ORDER", lambda: ss.current_phase == SpiralPhase.FIRST_ORDER_OBSERVATION
)
...
packet = prepare_agent_packet(
    {"step": 1, "episode": 5, "action": "web_search", "outcome": "success", "confidence": 0.95}
)
...
run_check(
    "Event has severity", lambda: hasattr(events[0], "severity") and events[0].severity is not None
)
...
# Run a simulation
event = {"type": "file_operation", "path": "/test/data", "action": "reorganize", "file_count": 50}
...
run_check(
    "Outcomes have probability", lambda: all(0 <= o.probability <= 1 for o in prediction.outcomes)
)
run_check(
    "Outcomes have reversibility",
    lambda: all(0 <= o.reversibility <= 1 for o in prediction.outcomes),
)
...
run_check(
    "record_insight returns path", lambda: insight_path is not None and len(str(insight_path)) > 0
)
