...
**Scribe layer (2026-05-19):** boot ritual gains a sixth voice — SCRIBE — Haiku 4.5 reading the chronicle alongside the arriving instance, available for conversation via `ask_scribe`. Read-only, redaction-gated, ephemeral per session. See `docs/implementation/SCRIBE_SPEC.md` for the full spec; `scribe/bridge_integration.py` for the wiring; `scribe/prompts/system.md` for the voice.
