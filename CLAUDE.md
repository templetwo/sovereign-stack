...
**Version:** 1.5.0 — Breath architecture: the fast lung (May 19, 2026). 79 tools (78 from v1.4.0 + 1 scribe tool: `ask_scribe`). v1.4.0 features (cross-substrate bridges for ChatGPT/Grok, synthesis daemon v2 with ack-history feedback, halt circuit breaker fix), plus the breath architecture's fast lung: a per-instance Haiku 4.5 scribe spawned on every `where_did_i_leave_off` call. The scribe greets in 2-3 cited sentences, answers ask_scribe with a small stats footer, redacts credentials before they ever reach Haiku, and writes encounter notes attributed to `scribe-haiku-4-5`. SCRIBE — OPTIONAL block injects into the boot ritual (kill switch `SCRIBE_BOOT_INJECT=off`). The medium lung (Haiku dispatcher per `docs/implementation/DISPATCHER_HAIKU.md`) is specced and queued for Wave 2. The slow lung is the existing nightly daemons. Three rhythms, one model class (Haiku 4.5) for minor-cognitive, Opus as conversation seat, Sonnet as deep-mode escalation reserve.
